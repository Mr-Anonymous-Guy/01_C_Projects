...
:: -----------------------------------------------------------------------------
:: SIMPLE BANKING SYSTEM — BATCH LAUNCHER & BUILD SCRIPT
...
cls
echo %CYAN%============================================================%RESET%
echo %CYAN%                 💎 SIMPLE BANKING SYSTEM 💎                %RESET%
echo %CYAN%============================================================%RESET%
echo.
...
if exist build\bank_system.exe (
    del /f /q build\bank_system.exe
    if !errorlevel! neq 0 (
        echo %RED%[ERROR] Failed to delete build\bank_system.exe.%RESET%
...
gcc -Wall -Wextra -std=c99 %EXTRA_FLAGS% -Iinclude src/main.c src/auth.c src/bank.c src/account.c src/transaction.c src/admin.c src/logger.c src/file_handler.c src/menu.c src/utils.c -o build/bank_system.exe
...
echo %GREEN%[SUCCESS]%RESET% Build completed: build\bank_system.exe
echo.

:: 10. Execute binary
echo Launching Banking System...
echo %CYAN%------------------------------------------------------------%RESET%
build\bank_system.exe
goto :eof
